...
#ifdef CONFIG_HAS_EARLYSUSPEND
    client_data->early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 1;
    client_data->early_suspend.suspend = adp5587_early_suspend;
    client_data->early_suspend.resume = adp5587_late_resume;
    register_early_suspend(&client_data->early_suspend);
#endif
	return 0;
...
exit_clear_interrupt_and_enable_irq:
    ...
    i2c_smbus_write_byte_data(client,INT_MODE_CFG,KE_IEN);  
...
{
	int ret;
	struct adp5587_data *adp5587 = i2c_get_clientdata(client);
	disable_irq(client->irq);
	ret = cancel_work_sync(&adp5587->work);
	ret = i2c_smbus_write_byte_data(adp5587->client, INT_MODE_CFG,KE_IDISABLE);   /* disable interrupt */
	if (ret < 0)
    {   
		KEYPAD_DEBUG(KERN_ERR "adp5587_suspend disable interrupt failed\n");
	}
	return 0;
...
MODULE_AUTHOR("developer <developer@huawei.com>");
